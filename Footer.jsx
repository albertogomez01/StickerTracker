...
export default function Footer() {
  ...
  const getButtonClass = (seccion) => {
    const isActive = location.pathname === seccion || (seccion === '/intercambios' && location.pathname === '/');
    return `footer-nav-btn ${isActive ? 'footer-nav-btn--active' : ''}`;
  };

  return (
    <div className="footer-nav">
      <button onClick={() => navigate('/album')} className={`${getButtonClass('/album')} tour-album`}>
      ...
      <button onClick={() => navigate('/importar')} className={`${getButtonClass('/importar')} tour-importar`}>
        <span className="footer-nav-btn__icon">
      ...
      <button onClick={() => navigate('/intercambios')} className={`${getButtonClass('/intercambios')} tour-intercambios`}>
        <span className="footer-nav-btn__icon">
        ...
        <span className="footer-nav-btn__label">Amigos</span>
      </button>
      <button onClick={() => navigate('/mercado')} className={`${getButtonClass('/mercado')} tour-mercado`}>
        <span className="footer-nav-btn__icon">
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg>
        </span>
        <span className="footer-nav-btn__label">Mercado</span>
      </button>
      <button onClick={() => navigate('/stats')} className={`${getButtonClass('/stats')} tour-stats`}>
        <span className="footer-nav-btn__icon">
